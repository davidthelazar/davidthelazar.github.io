...
function c_AlexendlessRunnarah(){
	c_App.call(this);
	this.m_resolution=null;
	this.m_assetManager=null;
	this.m_controller=null;
	this.m_camera=null;
	this.m_platformStack=c_GameObjectStack.m_new.call(new c_GameObjectStack);
	this.m_shotStack=c_GameObjectStack.m_new.call(new c_GameObjectStack);
	this.m_enemyStack=c_GameObjectStack.m_new.call(new c_GameObjectStack);
	this.m_clientStack=c_GameObjectStack2.m_new.call(new c_GameObjectStack2);
	this.m_billboardStack=c_GameObjectStack.m_new.call(new c_GameObjectStack);
	this.m_menu=c_UIMenu.m_new.call(new c_UIMenu);
	this.m_spawnTimer=c_Timer.m_new.call(new c_Timer);
	this.m_platformTimer=c_Timer.m_new.call(new c_Timer);
	this.m_score=0;
	this.m_player=null;
	this.m_firstPrint=true;
	this.m_ground=c_GameObjectStack.m_new.call(new c_GameObjectStack);
	this.m_groundHeight=650.0;
}
c_AlexendlessRunnarah.prototype=extend_class(c_App);
c_AlexendlessRunnarah.m_new=function(){
	c_App.m_new.call(this);
	return this;
}
c_AlexendlessRunnarah.m_Data=null;
c_AlexendlessRunnarah.prototype.p_InitMainMenu=function(){
	var t_playButton=c_UIButton.m_new.call(new c_UIButton,600.0,600.0,false);
	...
	var t_callback=c_PlayButtonCallBack.m_new.call(new c_PlayButtonCallBack);
	t_playButton.m_callBack=(t_callback);
	this.m_menu.p_AddButton(t_playButton);
	return 0;
}
c_AlexendlessRunnarah.prototype.p_OnCreate=function(){
	bb_alexendlessrunnarah_PauseFont=c_TFont.m_new.call(new c_TFont,"joystix monospace.ttf",100,[0,0,0]);
	bb_alexendlessrunnarah_ScoreFont=c_TFont.m_new.call(new c_TFont,"joystix monospace.ttf",40,[0,0,0]);
	bb_app_SetUpdateRate(60);
	bb_random_Seed=lazeng.systemMillisecs();
	this.m_resolution=c_Vector.m_new.call(new c_Vector,1334.0,750.0);
	...
	c_AlexendlessRunnarah.m_Data=this;
	bb_alexendlessrunnarah_LoadAllImages();
	...
	this.m_platformStack.p_InitPool();
	this.m_platformStack.m_camera=this.m_camera;
	this.m_shotStack.p_InitPool();
	this.m_shotStack.m_camera=this.m_camera;
	this.m_enemyStack.p_InitPool();
	this.m_enemyStack.m_camera=this.m_camera;
	this.m_clientStack.p_InitPool();
	this.m_clientStack.m_camera=this.m_camera;
	this.m_billboardStack.p_InitPool();
	this.m_billboardStack.m_camera=this.m_camera;
	this.p_InitMainMenu();
	return 0;
}
c_AlexendlessRunnarah.prototype.p_OnUpdate=function(){
	if(bb_alexendlessrunnarah_gameMode==1){
		this.m_spawnTimer.p_Update();
		this.m_platformTimer.p_Update();
		if(this.m_spawnTimer.m_alarm){
			var t_test0=bb_random_Rnd3(1.0);
			this.m_spawnTimer.p_Set9(60);
			var t_clientTemp=c_Client.m_NewFromPool(this.m_clientStack.p_GetPool(),"0",((this.m_camera.p_Right()+60.0)|0),((t_test0*(this.m_camera.p_Height()-100.0))|0));
			t_clientTemp.m_velocity.p_x(2.0);
			t_clientTemp.m_offScreenBehavior.p_Left3("kill");
			t_clientTemp.m_offScreenBehavior.p_Right3("update");
			t_clientTemp.m_offScreenBehavior.p_Above("kill");
			t_clientTemp.m_offScreenBehavior.p_Below("kill");
			t_clientTemp.p_HP(1.0);
			t_clientTemp.m_team=2;
			this.m_clientStack.p_Add2(t_clientTemp);
		}
		if(this.m_platformTimer.m_alarm){
			var t_test02=bb_random_Rnd3(1.0);
			this.m_platformTimer.p_Set9(120);
			var t_platformTemp=c_GameObject.m_NewFromPool(this.m_platformStack.p_GetPool(),bb_alexendlessrunnarah_GetAssetManager().p_GetImage("ground1"),this.m_camera.p_Right()+251.0,t_test02*(this.m_camera.p_Height()-100.0),true,true);
			t_platformTemp.m_velocity.p_x(0.0);
			t_platformTemp.m_offScreenBehavior.p_Left3("kill");
			t_platformTemp.m_offScreenBehavior.p_Right3("freeze");
			t_platformTemp.m_offScreenBehavior.p_Above("kill");
			t_platformTemp.m_offScreenBehavior.p_Below("kill");
			this.m_platformStack.p_Add(t_platformTemp);
		}
		var t_billboardDraws=((this.m_score/100)|0);
		for(var t_idx=0;t_idx<t_billboardDraws;t_idx=t_idx+1){
			var t_rando=bb_random_Rnd3(1.0);
			if(t_rando<.01){
				var t_test03=bb_random_Rnd3(1.0);
				var t_bbTemp=c_GameObject.m_NewFromPool(this.m_billboardStack.p_GetPool(),bb_alexendlessrunnarah_GetAssetManager().p_GetImage("spr_ShunnarahBillboard"),this.m_camera.p_Right()+251.0,t_test03*(this.m_camera.p_Height()-100.0),true,false);
				t_bbTemp.m_velocity.p_x(0.0);
				t_bbTemp.m_offScreenBehavior.p_Left3("kill");
				t_bbTemp.m_offScreenBehavior.p_Right3("freeze");
				t_bbTemp.m_offScreenBehavior.p_Above("kill");
				t_bbTemp.m_offScreenBehavior.p_Below("kill");
				t_bbTemp.m_offScreenBehavior.p_OnScreen("freeze");
				this.m_billboardStack.p_Add(t_bbTemp);
			}
		}
		this.m_controller.p_Update();
		this.m_camera.p_Update();
		this.m_menu.p_Update();
		this.m_shotStack.p_Update();
		this.m_clientStack.p_Update();
		this.m_billboardStack.p_Update();
		for(var t_touchIndex=0;t_touchIndex<10;t_touchIndex=t_touchIndex+1){
			if(bb_input_TouchHit(t_touchIndex)>0){
				var t_touchPoint=this.m_camera.p_Screen2World(c_Vector.m_new.call(new c_Vector,bb_autofit_VTouchX(t_touchIndex,true),bb_autofit_VTouchY(t_touchIndex,true)));
				for(var t_clientIndex=0;t_clientIndex<this.m_clientStack.p_Length2();t_clientIndex=t_clientIndex+1){
					var t_currItem=this.m_clientStack.p_Get2(t_clientIndex);
					if(t_currItem.p_State2()==1 || t_currItem.p_State2()==2){
						if(t_currItem.p_HitBox().p_ContainsPoint(t_touchPoint)){
							print(String(t_currItem.p_State2()));
							this.m_score=this.m_score+t_currItem.m_points;
							if(t_currItem.p_State2()==1){
								t_currItem.p_State(3);
								print(String(t_currItem.p_State2()));
							}else{
								if(t_currItem.p_State2()==2){
									t_currItem.p_State(4);
									print(String(t_currItem.p_State2()));
								}
							}
						}
		...
		if(this.m_player!=null){
			...
			if(this.m_player.p_IsDead()){
				this.m_player=null;
			}
		}
		bb_alexendlessrunnarah_HandleCollisions(this.m_player,this.m_enemyStack,this.m_shotStack,this.m_platformStack);
		if(this.m_controller.p_JustPressed("START")){
			bb_alexendlessrunnarah_gameMode=3;
		}
	}else{
		if(bb_alexendlessrunnarah_gameMode==3){
			this.m_controller.p_Update();
			if(this.m_controller.p_JustPressed("START")){
				bb_alexendlessrunnarah_gameMode=1;
			}
		}else{
			if(bb_alexendlessrunnarah_gameMode==2){
			}
		}
	}
	return 0;
}
c_AlexendlessRunnarah.prototype.p_OnRender=function(){
	if(this.m_firstPrint){
		bb_alexendlessrunnarah_PauseFont.p_DrawText("Test String",10,0,0,0,0,0);
		bb_alexendlessrunnarah_ScoreFont.p_DrawText("Test String",10,0,0,0,0,0);
	...
	if(bb_alexendlessrunnarah_gameMode==1){
		...
		this.m_billboardStack.p_Render();
		this.m_ground.p_Render();
		this.m_platformStack.p_Render();
		this.m_shotStack.p_Render();
		this.m_clientStack.p_Render();
		if(this.m_player!=null){
			this.m_player.p_Render();
		}
		bb_alexendlessrunnarah_ScoreFont.p_DrawText(String(this.m_score),1200,700,0,0,0,0);
		...
		if(bb_alexendlessrunnarah_gameMode==3){
			bb_autofit_UpdateVirtualDisplay(true,true);
			bb_graphics_Cls(123.0,123.0,255.0);
			bb_alexendlessrunnarah_PauseFont.p_DrawText("PAUSED!",700,300,0,0,0,0);
		}else{
			if(bb_alexendlessrunnarah_gameMode==2){
			}
		}
...
c_AlexendlessRunnarah.prototype.p_InitPlayer=function(t_spawnVect){
	t_spawnVect=this.m_camera.p_Screen2World(t_spawnVect);
	this.m_player=c_Player.m_new.call(new c_Player,t_spawnVect.p_x2(),t_spawnVect.p_y2(),c_AlexendlessRunnarah.m_Data.m_controller,this.m_shotStack);
	this.m_player.m_acceleration=this.m_player.p_Gravity();
	return 0;
}
c_AlexendlessRunnarah.prototype.p_InitLevel=function(t_levelNo){
	var t_levelLength=133400;
	var t_tempImage=this.m_assetManager.p_GetImage("ground1");
	for(var t_idx=0;t_idx<1334;t_idx=t_idx+1){
		var t_groundTemp=c_GameObject.m_new.call(new c_GameObject,t_tempImage,(250+t_idx*500),this.m_groundHeight,true,true);
		t_groundTemp.m_isCollideable=false;
		this.m_ground.p_Push7(t_groundTemp);
	}
	this.m_spawnTimer=c_Timer.m_new2.call(new c_Timer,60);
	this.m_platformTimer=c_Timer.m_new2.call(new c_Timer,120);
	return 0;
}
c_AlexendlessRunnarah.prototype.p_StartGame=function(){
	this.p_InitPlayer(c_Vector.m_new.call(new c_Vector,360.0,-100.0));
	this.m_player.m_velocity.p_x(this.m_player.m_moveSpeed);
	this.m_camera.m_velocity.p_x(this.m_player.m_moveSpeed);
	this.p_InitLevel(1);
	return 0;
}
...
function bbMain(){
	c_AlexendlessRunnarah.m_new.call(new c_AlexendlessRunnarah);
	return 0;
}
...
var bb_alexendlessrunnarah_PauseFont=null;
var bb_alexendlessrunnarah_ScoreFont=null;
...
c_Vector.prototype.p_Angle2=function(t_theta){
	...
	this.p_Set3(t_mag*Math.cos((t_theta)*D2R),t_mag*Math.sin((t_theta)*D2R));
	return 0;
}
...
c_VirtualDisplay.prototype.p_VTouchY=function(t_index,t_limit){
	var t_touchoffset=bb_input_TouchY(t_index)-(bb_app_DeviceHeight())*0.5;
...
c_AssetManager.prototype.p_AddImage=function(t_img,t_name){
	this.m_images.p_Set4(t_name,t_img);
	return 0;
}
...
c_AssetManager.prototype.p_AddSound=function(t_snd,t_name){
	this.m_sounds.p_Set6(t_name,t_snd);
	return 0;
}
...
c_AssetManager.prototype.p_GetImage=function(t_path){
	return this.m_images.p_Get(t_path);
}
c_AssetManager.prototype.p_GetSound=function(t_path){
	return this.m_sounds.p_Get(t_path);
}
function c_Map2(){
	Object.call(this);
	this.m_root=null;
...
c_Map2.prototype.p_Set4=function(t_key,t_value){
	var t_node=this.m_root;
	var t_parent=null;
	var t_cmp=0;
	while((t_node)!=null){
...
c_Map4.prototype.p_Set6=function(t_key,t_value){
	var t_node=this.m_root;
	var t_parent=null;
	var t_cmp=0;
	while((t_node)!=null){
...
function bb_alexendlessrunnarah_LoadAllImages(){
	var t__am=c_AlexendlessRunnarah.m_Data.m_assetManager;
	t__am.p_LoadImage("sprMissing",2);
	t__am.p_LoadImage("sprPlayer",3);
	t__am.p_LoadImage("sprMenuButton",4);
	t__am.p_LoadImage("ground1",1);
	t__am.p_LoadImage("background1",1);
	t__am.p_LoadImage("sprProjStock",10);
	t__am.p_LoadImage("sprEnemyCircleL1",1);
	t__am.p_LoadImage("spr_ShunnarahBillboard",1);
	t__am.p_LoadSound("pew","wav");
	t__am.p_LoadSound("rip","wav");
	t__am.p_LoadSound("shploing","wav");
	return 0;
}
...
c_Controller.prototype.p_InitDefaultMapping=function(t_pNo){
	...
	if(t_pNo==0){
		this.m__mapping.p_Set5("LEFT",[264,65]);
		this.m__mapping.p_Set5("UP",[265,87]);
		this.m__mapping.p_Set5("RIGHT",[266,68]);
		this.m__mapping.p_Set5("DOWN",[267,83]);
		this.m__mapping.p_Set5("A",[256,32]);
		this.m__mapping.p_Set5("B",[257,90]);
		this.m__mapping.p_Set5("X",[258,88]);
		this.m__mapping.p_Set5("Y",[259,67]);
		this.m__mapping.p_Set5("START",[263,13]);
		this.m__mapping.p_Set5("SELECT",[262,222]);
		this.m__mapping.p_Set5("LTRIG",[260,81]);
		this.m__mapping.p_Set5("RTRIG",[261,69]);
		this.m__mapping.p_Set5("1",[49]);
		this.m__mapping.p_Set5("2",[50]);
		this.m__mapping.p_Set5("3",[51]);
		this.m__mapping.p_Set5("4",[52]);
		this.m__mapping.p_Set5("5",[53]);
		this.m__mapping.p_Set5("6",[54]);
		this.m__mapping.p_Set5("7",[55]);
		this.m__mapping.p_Set5("8",[56]);
		this.m__mapping.p_Set5("9",[57]);
		this.m__mapping.p_Set5("0",[48]);
	}
	return 0;
}
c_Controller.prototype.p_InitInitialInput=function(t_pNo){
	...
	while(t_.p_HasNext()){
		...
		this.m__input.p_Set7(t_key,false);
		this.m__justPressed.p_Set7(t_key,false);
		this.m__justReleased.p_Set7(t_key,false);
	}
	return 0;
}
...
c_Controller.prototype.p_CheckControl=function(t_str){
	var t_allValidInputs=this.m__mapping.p_Get(t_str);
...
c_Controller.prototype.p_Update=function(){
	...
	while(t_.p_HasNext()){
		...
		t_pressedThen=this.m__input.p_Get(t_key);
		if(t_pressedThen){
			if(t_pressedNow){
				this.m__justReleased.p_Set7(t_key,false);
				this.m__justPressed.p_Set7(t_key,false);
			}else{
				this.m__justReleased.p_Set7(t_key,true);
				this.m__justPressed.p_Set7(t_key,false);
			}
		}else{
			if(t_pressedNow){
				this.m__justReleased.p_Set7(t_key,false);
				this.m__justPressed.p_Set7(t_key,true);
			}else{
				this.m__justReleased.p_Set7(t_key,false);
				this.m__justPressed.p_Set7(t_key,false);
			}
		}
		this.m__input.p_Set7(t_key,t_pressedNow);
	}
	var t_R=this.m__input.p_Get("RIGHT");
	var t_L=this.m__input.p_Get("LEFT");
	var t_U=this.m__input.p_Get("UP");
	var t_D=this.m__input.p_Get("DOWN");
	this.p_ProcessDirection(t_R,t_L,t_U,t_D);
	return 0;
}
c_Controller.prototype.p_JustPressed=function(t_str){
	return this.m__justPressed.p_Get(t_str);
}
c_Controller.prototype.p_IsDirectionPressed=function(){
	return this.m__isDirectionPressed;
}
c_Controller.prototype.p_GetDirection=function(){
...
c_Map5.prototype.p_Set7=function(t_key,t_value){
	var t_node=this.m_root;
	var t_parent=null;
	var t_cmp=0;
	while((t_node)!=null){
...
c_Camera.prototype.p_Screen2World=function(t_point){
	return t_point.p_Plus(this.p_TL3());
}
...
c_GameObject.m_NewFromPool=function(t__pool,t__img,t_x,t_y,t_willBeVisible,t_willBeCollideable){
	var t_temp=t__pool.p_Allocate().p_setDefaults(t__img,t_willBeVisible,t_willBeCollideable);
	t_temp.m_position=c_Vector.m_new.call(new c_Vector,t_x,t_y);
	return t_temp;
}
...
c_GameObject.prototype.p_IncrementTimers=function(){
	...
	while(t_.p_HasNext()){
		var t_key=t_.p_NextObject();
		this.m_timers.p_Get2(t_key).p_Update();
		if(this.m_timers.p_Get2(t_key).m_alarm){
...
c_GameObject.prototype.p_Reset=function(){
	this.m_sprite=null;
	this.m_position=null;
	this.m_velocity=null;
	this.m_acceleration=null;
	this.m_isCollideable=false;
	this.m_timers=null;
	this.m_offScreenBehavior=null;
	this.m_maxHP=0.0;
	this.m_team=0;
	...
	this.m__HP=0.0;
	this.m__isDead=false;
	this.m__isOnScreen=false;
	return 0;
}
c_GameObject.prototype.p_ReturnToPool=function(t__pool){
	this.p_Reset();
	t__pool.p_Free(this);
	return 0;
}
c_GameObject.prototype.p_AddTimer=function(t_whichTimer,t_howManyUpdates){
	var t_t=c_Timer.m_new2.call(new c_Timer,t_howManyUpdates);
	this.m_timers.p_Set10(t_whichTimer,t_t);
	return 0;
}
c_GameObject.prototype.p_SetTimer=function(t_whichTimer,t_howManyUpdates){
	var t_thisTimer=this.m_timers.p_Get2(t_whichTimer);
	if(t_thisTimer!=null){
		t_thisTimer.p_Set9(t_howManyUpdates);
	}else{
		this.p_AddTimer(t_whichTimer,t_howManyUpdates);
	}
	return 0;
}
c_GameObject.prototype.p_Bottom=function(){
	return this.m_position.p_y2()+this.m_sprite.p_Height()/2.0;
}
...
c_GameObject.prototype.p_Right=function(){
	return this.m_position.p_x2()+this.m_sprite.p_Width()/2.0;
}
c_GameObject.prototype.p_Left=function(){
	return this.m_position.p_x2()-this.m_sprite.p_Width()/2.0;
}
c_GameObject.prototype.p_Top=function(){
	return this.m_position.p_y2()-this.m_sprite.p_Height()/2.0;
}
...
function c_GameObjectStack(){
	c_Stack3.call(this);
	this.m_removeStack=null;
	this.m__pool=null;
	this.m_camera=null;
}
c_GameObjectStack.prototype=extend_class(c_Stack3);
c_GameObjectStack.m_new=function(){
	c_Stack3.m_new.call(this);
	this.m_removeStack=c_IntStack.m_new2.call(new c_IntStack);
	return this;
}
c_GameObjectStack.prototype.p_InitPool=function(){
	this.m__pool=c_Pool.m_new.call(new c_Pool,10);
	return 0;
}
c_GameObjectStack.prototype.p_GetPool=function(){
	if(this.m__pool!=null){
		return this.m__pool;
	}else{
		return null;
	}
}
c_GameObjectStack.prototype.p_Add=function(t_item){
	this.p_Push7(t_item);
	return 0;
}
c_GameObjectStack.prototype.p_Update=function(){
	for(var t_idx=0;t_idx<this.p_Length2();t_idx=t_idx+1){
		var t_currItem=this.p_Get2(t_idx);
		t_currItem.p_Update();
		if(t_currItem.p_IsDead()){
			this.m_removeStack.p_Push10(t_idx);
		}
	}
	while(!this.m_removeStack.p_IsEmpty()){
		var t_idx2=this.m_removeStack.p_Pop();
		if(this.m__pool!=null){
			this.p_Get2(t_idx2).p_ReturnToPool(this.m__pool);
		}
		this.p_Remove(t_idx2);
	}
	return 0;
}
c_GameObjectStack.prototype.p_Render=function(){
	for(var t_idx=0;t_idx<this.p_Length2();t_idx=t_idx+1){
		this.p_Get2(t_idx).p_Render();
	}
	return 0;
}
function c_Stack4(){
...
c_IntStack.prototype=extend_class(c_Stack4);
...
function c_Pool(){
	Object.call(this);
	this.m__pool=c_Stack3.m_new.call(new c_Stack3);
}
c_Pool.m_new=function(t_initialCapacity){
	for(var t_i=0;t_i<t_initialCapacity;t_i=t_i+1){
		this.m__pool.p_Push7(c_GameObject.m_new.call(new c_GameObject,null,0.0,0.0,true,true));
	}
	return this;
}
c_Pool.prototype.p_Allocate=function(){
	if(this.m__pool.p_IsEmpty()){
		return c_GameObject.m_new.call(new c_GameObject,null,0.0,0.0,true,true);
	}
	return this.m__pool.p_Pop();
}
c_Pool.prototype.p_Free=function(t_t){
	this.m__pool.p_Push7(t_t);
}
function c_Sprite(){
	Object.call(this);
...
c_Sprite.prototype.p_AddAnimation=function(t_anim){
	this.m_animations.p_Set8(t_anim.m_name,t_anim);
	return 0;
}
...
c_Sprite.prototype.p_GetAnimation=function(t_name){
	return this.m_animations.p_Get(t_name);
}
...
c_Sprite.prototype.p_DrawAt=function(t_x,t_y,t_angle){
	if(this.m_isVisible && this.p_SpriteSheet2()!=null){
		bb_graphics_DrawImage2(this.p_SpriteSheet2(),t_x,t_y,t_angle,this.p_XScale2(),this.p_YScale2(),this.m_currentAnimation.p_CurrentFrame());
	}
	return 0;
}
...
c_Animation.prototype.p_CurrentFrame=function(){
	if(!this.m_pingPong){
		if(this.m_loop){
			...
			if((this.p_FirstCycleCompleted())!=0){
...
c_Map6.prototype.p_Set8=function(t_key,t_value){
	var t_node=this.m_root;
	var t_parent=null;
	var t_cmp=0;
	while((t_node)!=null){
...
c_Timer.prototype.p_Set9=function(t_howManyTicks){
	this.m_ticks=t_howManyTicks;
	return 0;
}
function c_Map7(){
...
function c_Client(){
	...
	this.m_points=-100;
	this.m_strength=0.0;
	this.m_baseSpeed=5.0;
	this.m_moveSpeed=5.0;
}
c_Client.prototype.p_HP=function(t_hp){
	if(t_hp<=this.m_maxHP){
		this.m__HP=t_hp;
	}else{
		this.m__HP=this.m_maxHP;
...
c_Client.prototype.p_HP2=function(){
	return this.m__HP;
}
c_Client.prototype.p_State=function(t_newState){
	if(this.m__state!=t_newState){
		this.m__state=t_newState;
		this.m_changedState=true;
	}
	return 0;
}
c_Client.prototype.p_State2=function(){
	return this.m__state;
}
c_Client.prototype.p_setDefaults2=function(){
	this.m_sprite=c_Sprite.m_new.call(new c_Sprite,c_AlexendlessRunnarah.m_Data.m_assetManager.p_GetImage("sprMissing"),true);
	this.m_previousPosition=c_Vector.m_new.call(new c_Vector,0.0,0.0);
	this.m_position=c_Vector.m_new.call(new c_Vector,0.0,0.0);
	this.m_velocity=c_Velocity.m_new.call(new c_Velocity,0.0,0.0);
	this.m_acceleration=c_Vector.m_new.call(new c_Vector,0.0,0.0);
	...
	this.m_maxHP=1.0;
	this.p_HP(this.m_maxHP);
	this.m_offScreenBehavior=c_OffScreenBehavior2.m_new.call(new c_OffScreenBehavior2);
	this.m_offScreenBehavior.p_SetAll("update");
	this.p_State(1);
	this.m_sprite.p_NewAnimation("justFine",2,1,1);
	this.m_sprite.p_SetAnimation("justFine");
	this.m_camera=c_AlexendlessRunnarah.m_Data.m_camera;
	this.m_team=2;
	return this;
}
c_Client.m_new=function(t_x,t_y,t__ID){
	this.p_setDefaults2();
	this.m_position=c_Vector.m_new.call(new c_Vector,t_x,t_y);
	this.m_ID=t__ID;
	return this;
}
c_Client.m_new2=function(){
	return this;
}
c_Client.prototype.p_AddTimer=function(t_whichTimer,t_howManyUpdates){
	var t_t=c_Timer.m_new2.call(new c_Timer,t_howManyUpdates);
	this.m_timers.p_Set10(t_whichTimer,t_t);
	return 0;
}
c_Client.m_NewFromPool=function(t__pool,t__ID,t_x,t_y){
	var t_temp=t__pool.p_Allocate().p_setDefaults2();
	t_temp.m_position=c_Vector.m_new.call(new c_Vector,(t_x),(t_y));
	t_temp.m_ID=t__ID;
	t_temp.p_AddTimer(0,((bb_random_Rnd3(240.0))|0));
	return t_temp;
}
c_Client.prototype.p_IsDead=function(){
	return this.m__isDead;
}
c_Client.prototype.p_Width=function(){
	return this.m_sprite.p_Width();
}
c_Client.prototype.p_Height=function(){
	return this.m_sprite.p_Height();
}
c_Client.prototype.p_HitBox=function(){
	var t_cornerVect=c_Vector.m_new.call(new c_Vector,this.p_Width()/2.0,this.p_Height()/2.0);
	var t_point1=this.m_position.p_Plus(t_cornerVect);
	var t_point2=this.m_position.p_Minus(t_cornerVect);
	return c_Rect.m_new.call(new c_Rect,t_point1,t_point2);
}
c_Client.prototype.p_IsOnScreen=function(){
	return this.m__isOnScreen;
}
c_Client.prototype.p_IsOnScreen2=function(t_yeahButIsIt){
	this.m__isOnScreen=t_yeahButIsIt;
	return 0;
}
c_Client.prototype.p_checkOffScreenBehavior=function(){
	var t_whatToDo="Idunno";
	if(!this.p_IsDead()){
		if(this.m_camera!=null){
			if(this.p_HitBox().p_IsRightOf(this.m_camera)){
...
c_Client.prototype.p_Kill=function(){
	this.m__isDead=true;
	return 0;
}
c_Client.prototype.p_TimerAction=function(t_whichTimer){
	var t_1=t_whichTimer;
	if(t_1==0){
		this.p_State(2);
	}
...
c_Client.prototype.p_IncrementTimers=function(){
	var t_=this.m_timers.p_Keys().p_ObjectEnumerator();
	while(t_.p_HasNext()){
		var t_key=t_.p_NextObject();
		this.m_timers.p_Get2(t_key).p_Update();
...
c_Client.prototype.p_SetTimer=function(t_whichTimer,t_howManyUpdates){
	var t_thisTimer=this.m_timers.p_Get2(t_whichTimer);
	if(t_thisTimer!=null){
		t_thisTimer.p_Set9(t_howManyUpdates);
	}else{
...
c_Client.prototype.p_Update=function(){
	if(!this.p_IsDead()){
		var t_whatToDo=this.p_checkOffScreenBehavior();
		if(t_whatToDo=="kill"){
			this.p_Kill();
			...
			if(t_whatToDo=="freeze"){
				...
				this.m_position.p_x(this.m_position.p_x2()+.5*(t_previousVelocity.p_x2()+this.m_velocity.p_x2()));
				this.m_position.p_y(this.m_position.p_y2()+.5*(t_previousVelocity.p_y2()+this.m_velocity.p_y2()));
				if(this.m_changedState){
					if(this.p_State2()==1){
						this.m_sprite.p_NewAnimation("justFine",0,1,1);
						this.m_sprite.p_SetAnimation("justFine");
						this.m_points=-100;
					}else{
						if(this.p_State2()==2){
							this.m_sprite.p_NewAnimation("needsRepresentation",1,1,1);
							this.m_sprite.p_SetAnimation("needsRepresentation");
							this.m_points=100;
						}else{
							if(this.p_State2()==3){
								this.m_sprite.p_NewAnimation("malpracticed",0,2,2);
								this.m_sprite.p_SetAnimation("malpracticed");
								this.p_SetTimer(0,-1);
								print("malpracticed");
							}else{
								if(this.p_State2()==4){
									this.m_sprite.p_NewAnimation("litigated",0,2,4);
									this.m_sprite.p_SetAnimation("litigated");
									print("litigated");
								}
							}
						}
...
c_Client.prototype.p_Reset=function(){
	this.m_sprite=null;
	this.m_position=null;
	this.m_velocity=null;
	this.m_acceleration=null;
	...
	this.m__isOnScreen=false;
	this.m__state=0;
	this.m_baseSpeed=0.0;
	this.m_moveSpeed=0.0;
	this.m_changedState=false;
	this.m_ID="default";
	this.m_points=0;
	return 0;
}
c_Client.prototype.p_ReturnToPool2=function(t__pool){
	this.p_Reset();
	t__pool.p_Free2(this);
	return 0;
}
c_Client.prototype.p_Render=function(){
	if(this.m_sprite!=null){
		this.m_sprite.p_DrawAt(this.m_position.p_x2(),this.m_position.p_y2(),0.0);
	}
	return 0;
...
c_Stack5.m_NIL=null;
c_Stack5.prototype.p_Pop=function(){
	this.m_length-=1;
	var t_v=this.m_data[this.m_length];
	this.m_data[this.m_length]=c_Stack5.m_NIL;
	return t_v;
}
...
c_Stack5.prototype.p_Get2=function(t_index){
	return this.m_data[t_index];
}
...
function c_GameObjectStack2(){
	c_Stack5.call(this);
	this.m_removeStack=null;
	this.m__pool=null;
	this.m_camera=null;
}
c_GameObjectStack2.prototype=extend_class(c_Stack5);
c_GameObjectStack2.m_new=function(){
	c_Stack5.m_new.call(this);
	this.m_removeStack=c_IntStack.m_new2.call(new c_IntStack);
	return this;
}
c_GameObjectStack2.prototype.p_InitPool=function(){
	this.m__pool=c_Pool2.m_new.call(new c_Pool2,10);
	return 0;
}
c_GameObjectStack2.prototype.p_GetPool=function(){
	if(this.m__pool!=null){
		return this.m__pool;
	}else{
		return null;
...
c_GameObjectStack2.prototype.p_Add2=function(t_item){
	this.p_Push13(t_item);
	return 0;
}
c_GameObjectStack2.prototype.p_Update=function(){
	for(var t_idx=0;t_idx<this.p_Length2();t_idx=t_idx+1){
		var t_currItem=this.p_Get2(t_idx);
		t_currItem.p_Update();
		if(t_currItem.p_IsDead()){
	...
	while(!this.m_removeStack.p_IsEmpty()){
		var t_idx2=this.m_removeStack.p_Pop();
		if(this.m__pool!=null){
			this.p_Get2(t_idx2).p_ReturnToPool2(this.m__pool);
		}
		this.p_Remove(t_idx2);
	}
	return 0;
}
c_GameObjectStack2.prototype.p_Render=function(){
	for(var t_idx=0;t_idx<this.p_Length2();t_idx=t_idx+1){
		this.p_Get2(t_idx).p_Render();
	}
	return 0;
...
function c_Pool2(){
	Object.call(this);
	this.m__pool=c_Stack5.m_new.call(new c_Stack5);
}
c_Pool2.m_new=function(t_initialCapacity){
	for(var t_i=0;t_i<t_initialCapacity;t_i=t_i+1){
		this.m__pool.p_Push13(c_Client.m_new2.call(new c_Client));
	}
	return this;
}
c_Pool2.prototype.p_Allocate=function(){
	if(this.m__pool.p_IsEmpty()){
		return c_Client.m_new2.call(new c_Client);
	}
	return this.m__pool.p_Pop();
}
c_Pool2.prototype.p_Free2=function(t_t){
	this.m__pool.p_Push13(t_t);
}
function c_OffScreenBehavior2(){
	Object.call(this);
	this.m__below="";
	this.m__above="";
	this.m__left="";
...
c_OffScreenBehavior2.m_new=function(){
	return this;
}
c_OffScreenBehavior2.prototype.p_SetAll=function(t_behavior){
	this.m__below=t_behavior;
	this.m__above=t_behavior;
	this.m__left=t_behavior;
	this.m__right=t_behavior;
...
c_OffScreenBehavior2.prototype.p_Left3=function(t_behavior){
	this.m__left=t_behavior;
	return 0;
}
c_OffScreenBehavior2.prototype.p_Left=function(){
	return this.m__left;
}
c_OffScreenBehavior2.prototype.p_Right3=function(t_behavior){
	this.m__right=t_behavior;
	return 0;
}
c_OffScreenBehavior2.prototype.p_Right=function(){
	return this.m__right;
}
c_OffScreenBehavior2.prototype.p_Above=function(t_behavior){
	this.m__above=t_behavior;
	return 0;
}
c_OffScreenBehavior2.prototype.p_Above2=function(){
	return this.m__above;
}
c_OffScreenBehavior2.prototype.p_Below=function(t_behavior){
	this.m__below=t_behavior;
	return 0;
}
c_OffScreenBehavior2.prototype.p_Below2=function(){
	return this.m__below;
}
...
c_PlayButtonCallBack.prototype.p_call=function(){
	c_AlexendlessRunnarah.m_Data.m_menu.p_Destroy();
	c_AlexendlessRunnarah.m_Data.p_StartGame();
}
function c_UIMenu(){
	Object.call(this);
	this.m_buttonStack=c_Stack6.m_new.call(new c_Stack6);
	this.m_textStack=c_Stack7.m_new.call(new c_Stack7);
	this.m_graphicStack=c_Stack8.m_new.call(new c_Stack8);
}
...
c_UIMenu.prototype.p_AddButton=function(t_butt){
	this.m_buttonStack.p_Push16(t_butt);
	return 0;
}
...
c_Stack8.prototype.p_IsEmpty=function(){
	return this.m_length==0;
}
...
var bb_alexendlessrunnarah_gameMode=0;
...
function bb_alexendlessrunnarah_GetAssetManager(){
	return c_AlexendlessRunnarah.m_Data.m_assetManager;
}
...
function c_Player(){
	...
	this.m_shotStack=null;
}
c_Player.prototype=extend_class(c_GameObject);
c_Player.prototype.p_State=function(t_newState){
	if(this.m__state!=t_newState){
		this.m__state=t_newState;
		this.m_changedState=true;
	}
...
c_Player.prototype.p_State2=function(){
	return this.m__state;
}
c_Player.prototype.p_Update=function(){
	if(this.m_velocity.p_x2()==this.m_moveSpeed){
		this.m_acceleration.p_x(0.0);
	}
	var t_touchJump=false;
	if(this.p_State2()==2){
		for(var t_idx=0;t_idx<10;t_idx=t_idx+1){
			if((bb_input_TouchHit(t_idx))!=0){
				if(bb_autofit_VTouchX(t_idx,true)<200.0){
					t_touchJump=true;
					break;
				}
			}
		}
		if(this.m_controller.p_JustPressed("A") || t_touchJump){
			this.p_State(5);
			this.m_velocity.p_y(-20.0);
			bb_audio_PlaySound(bb_alexendlessrunnarah_GetAssetManager().p_GetSound("shploing"),4,0);
		}
	}
	if(!this.m_coolingDown){
		if(this.m_controller.p_IsDirectionPressed() || ((bb_input_TouchDown(0))!=0) && bb_autofit_VTouchX(0,true)<200.0){
			var t_tempVel=c_Velocity.m_new.call(new c_Velocity,10.0,0.0);
			if(this.m_controller.p_IsDirectionPressed()){
				t_tempVel.p_Direction2(this.m_controller.p_GetDirection());
			}else{
				if((bb_input_TouchDown(0))!=0){
					var t_MaxTheta=90.0;
					var t_PercentOfScreen=1.0;
					var t_Number=7.0;
					if(bb_autofit_VTouchY(0,true)<(1.0-t_PercentOfScreen)*bb_autofit_VDeviceHeight()/2.0){
						t_tempVel.p_Direction2(t_MaxTheta);
					}else{
						if(bb_autofit_VTouchY(0,true)>bb_autofit_VDeviceHeight()-(1.0-t_PercentOfScreen)*bb_autofit_VDeviceHeight()/2.0){
							t_tempVel.p_Direction2(-t_MaxTheta);
						}else{
							t_tempVel.p_Direction2(-t_MaxTheta*(bb_lazeng_Round((bb_autofit_VTouchY(0,true)-(1.0-t_PercentOfScreen)*bb_autofit_VDeviceHeight()/2.0)/(t_PercentOfScreen*bb_autofit_VDeviceHeight())-0.5+1.0/t_Number,1.0/t_Number)-1.0/t_Number));
						}
					}
				}
			}
			var t_tempShot=c_GameObject.m_NewFromPool(this.m_shotStack.p_GetPool(),bb_alexendlessrunnarah_GetAssetManager().p_GetImage("sprProjStock"),this.m_position.p_x2()+this.m_sprite.p_Width()/2.0*Math.cos((t_tempVel.p_Direction())*D2R),this.m_position.p_y2()-this.m_sprite.p_Height()/2.0*Math.sin((t_tempVel.p_Direction())*D2R),true,true);
			t_tempVel.p_x(t_tempVel.p_x2()+this.m_velocity.p_x2());
			t_tempShot.m_velocity=t_tempVel;
			t_tempShot.m_offScreenBehavior.p_Left3("kill");
			t_tempShot.m_offScreenBehavior.p_Right3("kill");
			t_tempShot.m_offScreenBehavior.p_Above("kill");
			t_tempShot.m_offScreenBehavior.p_Below("kill");
			t_tempShot.m_strength=1.0;
			t_tempShot.m_team=this.m_team;
			this.m_shotStack.p_Add(t_tempShot);
			this.p_SetTimer(0,10);
			this.m_coolingDown=true;
			bb_audio_PlaySound(bb_alexendlessrunnarah_GetAssetManager().p_GetSound("pew"),2,0);
		}
	}
	c_GameObject.prototype.p_Update.call(this);
	if(this.p_Bottom()>635.0){
		this.m_position.p_y(635.0-this.p_Height()/2.0);
		this.m_velocity.p_y(0.0);
		if(this.p_State2()==5){
			this.p_State(2);
		}else{
			if(this.p_State2()==4){
...
c_Player.prototype.p_Render=function(){
	c_GameObject.prototype.p_Render.call(this);
	return 0;
}
c_Player.m_new=function(t_x,t_y,t__c,t__s){
	c_GameObject.m_new.call(this,null,t_x,t_y,true,true);
	this.m_maxHP=10.0;
	this.p_HP(this.m_maxHP);
	this.m_sprite=c_Sprite.m_new.call(new c_Sprite,c_AlexendlessRunnarah.m_Data.m_assetManager.p_GetImage("sprPlayer"),true);
	this.m_controller=t__c;
	this.p_State(5);
	this.m_camera=c_AlexendlessRunnarah.m_Data.m_camera;
	this.m_shotStack=t__s;
	this.m_team=1;
	return this;
}
...
c_Player.prototype.p_Gravity=function(){
	return c_Vector.m_new.call(new c_Vector,0.0,0.5);
}
c_Player.prototype.p_TimerAction=function(t_whichTimer){
	if(t_whichTimer==0){
		this.m_coolingDown=false;
	}
	return 0;
...
c_Player.prototype.p_Kill=function(){
	c_GameObject.prototype.p_Kill.call(this);
	return 0;
}
...
function bb_lazeng_Round(t_flot,t_quantum){
	return t_quantum*Math.floor(t_flot/t_quantum+0.5);
}
function bb_alexendlessrunnarah_HandleCollisions(t_player,t_enemyStack,t_shotStack,t_platformStack){
	if(t_player!=null){
		if(!t_enemyStack.p_IsEmpty()){
			for(var t_idx=0;t_idx<t_enemyStack.p_Length2();t_idx=t_idx+1){
				var t_currItem=t_enemyStack.p_Get2(t_idx);
				if(t_currItem.p_IsOnScreen()){
					if(t_player.p_CollidesWith(t_currItem)){
		...
		if(!t_shotStack.p_IsEmpty()){
			for(var t_idx2=0;t_idx2<t_shotStack.p_Length2();t_idx2=t_idx2+1){
				var t_currItem2=t_shotStack.p_Get2(t_idx2);
				if(t_currItem2.p_IsOnScreen()){
					if(t_currItem2.m_team!=t_player.m_team){
						if(t_player.p_CollidesWith(t_currItem2)){
							print("PLAYER COLLIDES WITH PROJECTILE");
							t_player.p_HP(t_player.p_HP2()-t_currItem2.m_strength);
						}
					}
				}
			}
		}
		if(!t_platformStack.p_IsEmpty()){
			for(var t_idx3=0;t_idx3<t_platformStack.p_Length2();t_idx3=t_idx3+1){
				var t_currItem3=t_platformStack.p_Get2(t_idx3);
				var t_collidedWithPlatform=false;
				if(t_currItem3.p_IsOnScreen()){
					if(t_player.p_Right()>t_currItem3.p_Left()){
						if(t_player.p_Left()<t_currItem3.p_Right()){
							if(t_player.m_previousPosition.p_y2()+t_player.p_Height()/2.0<=t_currItem3.p_Top()){
								if(t_player.m_position.p_y2()+t_player.p_Height()/2.0>=t_currItem3.p_Top()){
									t_collidedWithPlatform=true;
									t_player.m_velocity.p_y(0.0);
									t_player.m_position.p_y(t_currItem3.p_Top()-t_player.p_Height()/2.0);
									if(t_player.p_State2()==4){
										t_player.p_State(1);
									}else{
										if(t_player.p_State2()==5){
											t_player.p_State(2);
										}
									}
								}
	...
	if(!t_enemyStack.p_IsEmpty()){
		for(var t_idx4=0;t_idx4<t_enemyStack.p_Length2();t_idx4=t_idx4+1){
			var t_currItem4=t_enemyStack.p_Get2(t_idx4);
			if(t_currItem4.p_IsOnScreen()){
				if(!t_shotStack.p_IsEmpty()){
					for(var t_kdx=0;t_kdx<t_shotStack.p_Length2();t_kdx=t_kdx+1){
						var t_nextItem=t_shotStack.p_Get2(t_kdx);
						if(t_nextItem.p_IsOnScreen()){
							if(t_currItem4.p_CollidesWith(t_nextItem)){
								print("ENEMY COLLIDES WITH PROJECTILE");
								t_currItem4.p_HP(t_currItem4.p_HP2()-t_nextItem.m_strength);
								t_nextItem.p_Kill();
								bb_audio_PlaySound(bb_alexendlessrunnarah_GetAssetManager().p_GetSound("rip"),3,0);
							}
						}
					}
				}
...
function bbInit(){
	...
	bb_graphics_renderDevice=null;
	bb_alexendlessrunnarah_PauseFont=null;
	bb_alexendlessrunnarah_ScoreFont=null;
