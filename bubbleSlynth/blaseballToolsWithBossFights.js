function digestSnapshots(event)
{
	...
	if (update.value.hasOwnProperty("games")) //is a normal game
	{
	...
	{
		games = update.value.fights.bossFights;
		var fightTemp = true;
	}
	let snapshots=[];
	...
	for (let idx = 0; idx < games.length; idx++) 
	{
		...
		snapshot.isFight = false;
		
		if (fightTemp)
		{
			snapshot.isFight = true;
			var damageArray = JSON.parse(thisGame.damageResults);
			snapshot.homeDamage = 0;
			snapshot.awayDamage = 0;
			if (damageArray.length!==0)
			{
				if (damageArray[0].teamTarget == thisGame.homeTeam)
				{
					snapshot.homeDamage = damageArray[0].dmg;
				}
				else
				{
					snapshot.awayDamage = damageArray[0].dmg;
				}
			}
		}
